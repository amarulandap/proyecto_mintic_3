...
app.get("/search-ventas", async (req,res)=> {
    const busqueda = req.query.busqueda;
    const valorBusqueda = req.query.valorBusqueda;
    const [rows, fields] = await connection.execute(`SELECT * FROM Ventas WHERE ${busqueda}='${valorBusqueda}'`);

    res.json({data: rows});
})

app.put("/update-ventas", async (req,res) => {
    try{const {IDventa, ValorVenta, Descripcion, FechaVenta, FechaPago, Responsable} = req.body;
    const [rows, fields] = await connection.execute(`UPDATE Ventas SET ValorVenta=${ValorVenta}, Descripcion='${Descripcion}', FechaVenta='${FechaVenta}', FechaPago='${FechaPago}', Responsable='${Responsable}' WHERE IDVenta='${IDventa}'`);
    res.json({status: "ok"});
    }catch (error){ 
    res.json(error);
}
